...
export class BoardHelper {
  ...
  static isLegalSelection(player: Player, square: Square): boolean {
    const playerMatchesSquare = player === square.player;
    const squareIsntKing = square.piece !== Piece.KING;
    return squareIsntKing && playerMatchesSquare;
  }
  ...
  static determineLegalMoves(board: Board, player: Player): Square[] {
    ...
    if (selectedSquare) {
      // find the squares the player can move to
      return board.squares
        .flat()
        .filter((square) => BoardHelper.isLegalMove(selectedSquare, square));
    } else {
      // find the squares the player can select
      return board.squares
        .flat()
        .filter((square) => BoardHelper.isLegalSelection(player, square));
    }
  }
}
